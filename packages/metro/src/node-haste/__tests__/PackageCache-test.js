...
describe('PackageCache', () => {
  describe('invalidate', () => {
    ...
    test('invalidating a module and then its package.json re-resolves both', () => {
      const closestPackages: ClosestPackageMap = new Map([
        [
          MODULE_A,
          {packageJsonPath: PKG_PATH, packageRelativePath: 'moduleA.js'},
    ...
    test('invalidating a module with a null result re-resolves it', () => {
      const closestPackages = new Map<
        string,
        ?{packageJsonPath: string, packageRelativePath: string},
      >([
    ...
    test('invalidating one package.json keeps the parsed contents of others', () => {
      const closestPackages: ClosestPackageMap = new Map([
        [
          MODULE_A,
          {packageJsonPath: PKG_PATH, packageRelativePath: 'moduleA.js'},
      ...
      // Module C's package.json was not re-read
      expect(cache.getPackageForModule(MODULE_C)?.packageJson.name).toBe(
        'pkg2',
      );
    });
